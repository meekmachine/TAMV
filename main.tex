...
\begin{itemize}
  \item \textbf{Evaluation methodology:} We assess TAMV extraction accuracy by comparing system output against gold annotations derived from Longman Grammar examples. \citep{Biber20101999LongmanEnglish} We report both per-dimension metrics (tense, aspect, mood, voice evaluated separately) and joint-label exact match to capture whether the complete TAMV tuple is correctly extracted. This mirrors the evaluation approach used by Ramm et al.\ for their TMV-annotator. \citep{Ramm2017AnnotatingGerman}

  \item \textbf{Test case construction approach:} Gold TAMV annotations are constructed in two stages:
    \begin{itemize}
      \item \emph{Stage 1---Example selection:} We manually extract verbal complexes from Longman Grammar's illustrative examples across all four registers (conversation, fiction, news, academic). These examples are canonical instances chosen by corpus linguists to demonstrate register-specific TAMV patterns. \citep{Biber20101999LongmanEnglish}
      \item \emph{Stage 2---Gold labeling:} Each verbal complex is manually annotated with its ground-truth TAMV values based on morphological and syntactic analysis. We follow the labeling conventions established by Ramm et al.\ \citep{Ramm2017AnnotatingGerman}, augmented with aspect distinctions (simple, progressive, perfect, perfect-progressive) not present in their TMV scheme.
      \item \emph{Inter-annotator agreement:} A subset of 50 test cases is independently annotated by two linguists to compute Cohen's $\kappa$ for each TAMV dimension, ensuring reliable gold labels.
    \end{itemize}
