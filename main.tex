...
\begin{itemize}
  ...
  \item \textbf{TAMV intrinsic evaluation:} Two complementary tracks: (i) benchmark against the annotated TMV/TAMV data from Ramm et al., and (ii) targeted sentence-level checks using examples selected (and when needed minimally adapted) from Longman Grammar to verify expected TAMV behavior across registers. \citep{Biber20101999LongmanEnglish,Ramm2017AnnotatingGerman}
...
\begin{itemize}
  \item \textbf{Evaluation methodology:} We run intrinsic TAMV evaluation in two ways. First, we compare extraction output against the annotated reference data released by Ramm et al.\ and report per-dimension scores (tense, aspect, mood, voice) plus joint-label exact match for full TAMV tuples. \citep{Ramm2017AnnotatingGerman} Second, we run targeted validation on sentence examples drawn from Longman Grammar (with minimal adaptations where needed) to check that extracted TAMV labels align with the grammatical patterns Longman documents across registers. \citep{Biber20101999LongmanEnglish}

  \item \textbf{Scope clarification:} Longman examples are used as linguistically motivated validation checks; they are not presented as a newly crowdsourced or multi-annotator gold corpus in this study.

  \item \textbf{Error taxonomy:} We categorize extraction errors into the following types to diagnose systematic failure modes:
    \begin{itemize}
      \item \emph{Parse errors:} Incorrect dependency structure from spaCy prevents correct auxiliary/main-verb identification.
      \item \emph{Tense errors:} Misidentification of past/present/future, often due to modal-future confusion (\emph{will} vs.\ present) or irregular verb forms.
      \item \emph{Aspect errors:} Confusion between progressive and perfect aspect markers, or failure to detect perfect-progressive combinations.
      \item \emph{Mood errors:} Modal polysemy (e.g., \emph{would} as past habitual vs.\ conditional) or subjunctive detection failures in \emph{that}-clause complements.
      \item \emph{Voice errors:} Passive/participle ambiguity (adjectival passives vs.\ verbal passives) or missing \texttt{nsubjpass} relation in UD parse.
      \item \emph{Clause segmentation errors:} Incorrect boundaries for verbal complexes, leading to missing or spurious TAMV annotations.
    \end{itemize}
\end{itemize}
...
\section{Results}
\begin{itemize}
  \item \textbf{TAMV reliability:} performance against Ramm et al.\ annotated reference data, plus consistency checks on Longman-derived examples, with major error sources reported. \citep{Biber20101999LongmanEnglish,Ramm2017AnnotatingGerman}
...
\begin{itemize}
  ...
  \item \textbf{Intrinsic evaluation findings:} Our extraction rules are evaluated against Ramm et al.\ annotated reference data \citep{Ramm2017AnnotatingGerman} and additionally checked on Longman-derived sentence examples \citep{Biber20101999LongmanEnglish}, supporting that UD-style dependency parses can recover TAMV patterns reliably. Tense and voice extraction show particularly robust performance, while mood/modality detection faces challenges in disambiguating modal polysemy and subjunctive contexts.
  ...
  \item \textbf{Future work:} Integrating temporal structure annotations \citep{Derczynski2012AnStructure} could resolve tense/aspect ambiguities. Expanding coverage to non-finite clauses would capture a fuller picture of verbal systems. Cross-linguistic extension using UD treebanks in other languages could test whether similar rule patterns generalize beyond English.
\end{itemize}
